...
module.exports = Ouro.Class.extend({
  ...
  //attach the ouro router middleware
  ...
  processError: function(err, req, res) {
    ...
    //todo: create different types of Ouro errors...

    //if super js error, the request has failed
    if( err instanceof Ouro.Error ) {
      self.log.warn('request failed:', {code: err.code, message: err.message});

      //else, an unknown error has occurred
    } else {
      err.status = 500;
      err = new Ouro.Error('unknown_error', 'An unknown error occured processing the request.', err);
    ...
    //move err.message to the meta section
    if( err.message ) {
